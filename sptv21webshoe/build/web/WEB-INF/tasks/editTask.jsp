...
<div class="w-100 d-flex justify-content-center">
    <form action="editTask?id=${id}" method="POST" >
        <div class="card border-0 mb-3 px-3" style="width: 40em;">
            ...
            <div class="form-group">
                ...
                <input type="text" class="form-control" name="name" value="${task.title}">
            </div>
            <div class="form-group">
                <label class="form-label mt-2 mx-2">Description</label>
                <textarea class="form-control" name="description" rows="3" value="">${task.description}</textarea>
            </div>
            <div class="form-group">
                <label class="form-label mt-2 mx-2">Task are done?</label>
                <select class="form-control" name="completed">
                    <option value="true" ${task.isCompleted() ? "selected" : ""}>Yes</option>
                    <option value="false" ${!task.isCompleted() ? "selected" : ""}>No</option>
                </select>
            </div>
